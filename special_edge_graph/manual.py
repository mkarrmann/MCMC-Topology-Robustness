...
def score_by_party(special_graph: SpecialEdgeGraph, party="A") -> SpecialEdgeGraph:
    ...
    def _score(node) -> float:
        if (
            special_graph.graph.nodes[node][CONFIG["PARTY_A_COL"]]
            + special_graph.graph.nodes[node][CONFIG["PARTY_B_COL"]]
            > 0
        ):
            return (
                (
...
def init_special_graphs(
    graph: Graph, score_func: Callable
) -> Tuple[List[SpecialEdgeGraph], float]:
    ...
    graphs = [deepcopy(special_graph) for _ in range(1)]  # TODO config number graphs
...
def init_parts(graph: Graph) -> List[Partition]:
    ...
    """
    parts = []
    total_pop = sum([graph.nodes[node][CONFIG["POP_COL"]] for node in graph.nodes()])
    ...
    with mp.Pool(processes=CONFIG["NUM_PROCESSES"]) as pool:
        ...
        results = pool.starmap(
            ...
            zip(
                ...
                (
                    f'{"Orig" if i % 2 == 1 else "Special"}{len(set(pair[0].values()))}'
                    for i, pair in enumerate(graph_part_pairs)
                ),
            ),
        )
